...
def home(prefix: str = "", **extra: object) -> dict:
    ...
    return provider("justdowork", prefix, ["messages"], ["claude-opus-5"],
...
for good in ("truesota--", "x--", "agent_router--", "truesota.anthropic.",
             "agent-router.anthropic.", "a.anthropic."):
    check(f"接受 {good!r}", bool(R.MODEL_PREFIX_PATTERN.fullmatch(good)))
for bad in ("anthropic.", ".anthropic.", "agent_router.anthropic.", "a.b.anthropic.",
            "truesota.anthropic", "truesota.anthropic..", "truesota-", "truesota",
            "--", "X--", "-x--", "truesota.openai."):
    check(f"拒绝 {bad!r}", not R.MODEL_PREFIX_PATTERN.fullmatch(bad))

print("\n=== 2) 派生：只说 messages 的才给点号形式 ===")
for pid, protos, want in [
    ("truesota", ["messages"], "truesota.anthropic."),
    ("agent_router", ["messages"], "agent-router.anthropic."),
    ("miaomiao", ["responses"], "miaomiao--"),
    ("docode", ["responses", "messages"], "docode--"),
    ("nobody", [], "nobody--"),
    ("nobody", None, "nobody--"),
]:
...
reg = validated(home(), provider("truesota", None, ["messages"], ["claude-opus-5"]))
by_id = {p["id"]: p for p in reg["providers"]}
check("没填 prefix 的 messages 供应商被补成点号形式",
      by_id["truesota"]["prefix"] == "truesota.anthropic.", by_id["truesota"]["prefix"])
check("默认那家的 prefix 还是空的", by_id["justdowork"]["prefix"] == "")
check("点号 slug 正常进发布列表",
      set(R.selectable_slugs(reg)) == {"claude-opus-5", "truesota.anthropic.claude-opus-5"},
      str(R.selectable_slugs(reg)))
rejects("默认那家不许带前缀", home(prefix="justdowork.anthropic."),
        provider("truesota", None, ["messages"], ["claude-opus-5"]))
rejects("两家不许撞同一个点号前缀", home(),
        provider("truesota", "same.anthropic.", ["messages"], ["claude-opus-5"]),
        provider("docode", "same.anthropic.", ["messages"], ["claude-opus-5"]))
...
rejects("默认那家的裸模型名撞上别人的点号 slug 也拦",
        provider("justdowork", "", ["messages"], ["truesota.anthropic.claude-opus-5"],
                 is_default=True),
        provider("truesota", None, ["messages"], ["claude-opus-5"]))
print("\n=== 4) 迁移：只动自己派生过的那几家，重跑无害 ===")
mixed = validated(
    home(),
    provider("truesota", "truesota--", ["messages"], ["claude-opus-5"]),
    provider("seekai", "seekai--", ["messages"], ["claude-opus-4-6"]),
    provider("docode", "docode--", ["responses", "messages"], ["claude-opus-5"]),
    provider("miaomiao", "miaomiao--", ["responses"], ["gpt-5.6-sol"]),
    provider("kktoken", "kk--", ["messages"], ["claude-opus-5"]),
)
moved = {pid: (old, new) for pid, old, new in R.upgrade_messages_prefixes(mixed)}
check("truesota 改成点号", moved.get("truesota") == ("truesota--", "truesota.anthropic."),
      str(moved.get("truesota")))
check("seekai 改成点号", moved.get("seekai") == ("seekai--", "seekai.anthropic."),
      str(moved.get("seekai")))
check("两端都说的 docode 不动", "docode" not in moved)
check("纯 responses 的 miaomiao 不动", "miaomiao" not in moved)
check("手写的 kk-- 不动：那是人挑的名字", "kktoken" not in moved)
check("默认那家不动，它得留着空前缀", "justdowork" not in moved)
...
for slug, want in [
    ("truesota.anthropic.claude-opus-5", "claude-opus-5"),
    ("agent-router.anthropic.claude-opus-4-8", "claude-opus-4-8"),
...
check("vendor-- 没人给它剥，这就是当初没有控件的全部原因",
      cd.thinking_effort_levels("truesota--claude-opus-5") is None)
check("点号形式落到表里了",
      cd.thinking_effort_levels("truesota.anthropic.claude-opus-5")
      == ("low", "medium", "high", "xhigh", "max"))
check("认得但只有开关的那种给空元组",
      cd.thinking_effort_levels("anthropic.claude-sonnet-4-5-v2:0") == ())
check("表里没有的名字还是 None",
      cd.thinking_effort_levels("truesota.anthropic.claude-opus-5-thinking") is None)
check("fable 家族走的是前缀匹配",
      cd.thinking_effort_levels("x.anthropic.claude-fable-5")
      == ("low", "medium", "high", "xhigh", "max"))
check("三种结论各有一句人话",
      [cd.thinking_summary(s) for s in ("truesota.anthropic.claude-opus-5",
                                        "anthropic.claude-sonnet-4-5",
                                        "truesota--claude-opus-5")]
      == ["思考档 low/medium/high/xhigh/max", "仅扩展思考开关", "无思考控件"],
      str([cd.thinking_summary(s) for s in ("truesota.anthropic.claude-opus-5",
                                           "anthropic.claude-sonnet-4-5",
                                           "truesota--claude-opus-5")]))
...
legacy = validated(
    ...
    provider("truesota", "truesota--", ["messages"], ["claude-opus-5"]),
    provider("seekai", "seekai--", ["messages"], ["claude-opus-4-6"]),
    provider("kktoken", "kktoken--", ["messages"], ["claude-opus-5-thinking"]),
)
blind_before = sorted(s for s in R.selectable_slugs(legacy)
                      if cd.thinking_effort_levels(s) is None)
check("改之前：三家的 slug 一个控件都没有",
      blind_before == ["kktoken--claude-opus-5-thinking", "seekai--claude-opus-4-6",
                       "truesota--claude-opus-5"], str(blind_before))
...
check("改之后只剩 -thinking 那个：档位写进了厂商自己的模型名，换前缀救不了它",
      blind_after == ["kktoken.anthropic.claude-opus-5-thinking"], str(blind_after))
check("seekai 拿到的是它那代的四档",
      cd.thinking_effort_levels("seekai.anthropic.claude-opus-4-6")
      == ("low", "medium", "high", "max"))
check("默认那家本来就有控件：空前缀 + 原厂模型名",
      cd.thinking_summary("claude-opus-5") == "思考档 low/medium/high/xhigh/max")

print("\n=== 7) 派发：点号 slug 照样解析回上游裸模型名 ===")
chain = R.failover_chain(legacy, "truesota.anthropic.claude-opus-5", protocol="messages")
check("解析到 truesota，发给上游的 id 不带前缀",
      list(chain)[:1] == [("truesota", "claude-opus-5")], str(chain))
stale = R.failover_chain(legacy, "truesota--claude-opus-5", protocol="messages")
check("老写法不再指向 truesota（迁移完就该这样）",
      all(pid != "truesota" for pid, _ in stale), str(stale))
wrong_wire = R.failover_chain(legacy, "truesota.anthropic.claude-opus-5", protocol="responses")
check("要 responses 时不给 truesota：它只会 messages",
      all(pid != "truesota" for pid, _ in wrong_wire), str(wrong_wire))
